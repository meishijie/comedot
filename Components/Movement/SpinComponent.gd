...
func _ready() -> void:
	...
	self.set_physics_process(isEnabled and nodeToRotate != null and not is_zero_approx(rotationPerFrame)) # Apply setters because Godot doesn't on initialization
